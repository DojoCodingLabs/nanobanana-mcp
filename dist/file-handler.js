 ...
 */
export function validatePath(inputPath, allowedDirs) {
    ...
    if (!isAllowed) {
        throw new Error(`Path "${inputPath}" is outside allowed directories. Allowed: ${allowedDirs.join(", ")}`);
    }
    return resolved;
}
/**
...
export async function readImageFile(filePath) {
    ...
    if (stats.size > MAX_FILE_SIZE_BYTES) {
        throw new Error(`File too large: ${Math.round(stats.size / 1024 / 1024)}MB exceeds ${MAX_FILE_SIZE_BYTES / 1024 / 1024}MB limit`);
    }
 ...
 */
export async function saveImage(base64Data, prefix) {
