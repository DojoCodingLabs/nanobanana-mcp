...
import os from "os";
import { GenerateImageArgsSchema, EditImageArgsSchema, ContinueEditingArgsSchema, } from "./types.js";
import { GeminiClient } from "./gemini-client.js";
import { validatePath, readImageFile, getMimeType, } from "./file-handler.js";
// --- Tool definitions ---
const TOOLS = [
    {
        name: "generate_image",
        description: "Generate a NEW image from a text prompt using Gemini. Use this ONLY when creating a completely new image, not when modifying an existing one.",
        inputSchema: {
            type: "object",
            properties: {
                prompt: {
    ...
    {
        name: "edit_image",
        description: "Edit an existing image file with a text prompt, optionally using additional reference images. Use this when you have the exact file path of an image to modify.",
        inputSchema: {
            type: "object",
            properties: {
                imagePath: {
                ...
                referenceImages: {
                    type: "array",
                    items: { type: "string" },
                    description: "Optional array of file paths to reference images (for style transfer, adding elements, etc.)",
                },
            },
    ...
    },
    {
        ...
        description: "Continue editing the LAST image generated or edited in this session. Automatically uses the previous image without needing a file path. Use for iterative improvements.",
        inputSchema: {
            type: "object",
            properties: {
                prompt: {
                    type: "string",
                    description: "Text describing changes to make to the last image (max 10,000 chars)",
                },
                referenceImages: {
                    type: "array",
                    items: { type: "string" },
...
class NanoBananaMCP {
    ...
    constructor() {
        ...
        this.server = new Server({ name: "nanobanana-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
        this.setupHandlers();
    }
    setupHandlers() {
    ...
    async handleGenerateImage(request) {
        const parsed = GenerateImageArgsSchema.safeParse(request.params.arguments);
        if (!parsed.success) {
            throw new McpError(ErrorCode.InvalidParams, parsed.error.errors.map((e) => e.message).join("; "));
        }
    ...
    async handleContinueEditing(request) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InvalidRequest, "Last image file no longer exists. Generate a new image first.");
        }
        const parsed = ContinueEditingArgsSchema.safeParse(request.params.arguments);
        if (!parsed.success) {
            throw new McpError(ErrorCode.InvalidParams, parsed.error.errors.map((e) => e.message).join("; "));
        }
        const { prompt, referenceImages } = parsed.data;
        return await this.editImageInternal(this.lastImagePath, prompt, referenceImages);
    }
    /**
     * Shared edit logic used by both edit_image and continue_editing.
     */
    async editImageInternal(imagePath, prompt, referenceImages) {
        ...
        const result = await this.gemini.editImage(imageBase64, mimeType, prompt, refData.length > 0 ? refData : undefined);
        if (!result.filePath) {
            return {
                content: [{ type: "text", text: result.textContent }],
            };
...
try {
    ...
    process.stderr.write(`Failed to initialize nanobanana-mcp: ${error instanceof Error ? error.message : error}\n`);
    process.exit(1);
}
